...
def main(simulation_length=2000, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, stored_times=None,
         ...
         INPUTS_PLANTSOIL_DIRPATH='inputs_umekaori_red', INPUT_METEO_DIRPATH='inputs_umekaori_red', METEO_FILENAME='meteo.csv', OUTPUTS_DIRPATH='outputs', POSTPROCESSING_DIRPATH='postprocessing', GRAPHS_DIRPATH='graphs'):
    """
    ...
    :param dict [int, float] or [str, float] N_fertilizations: a dictionary for N fertilisation regime {date: N_input}, with date in hour and N_input in ｵmol N nitrates
    ...
    if run_from_outputs:
        ...
        for initial_state_filename, outputs_filename, index_columns in ((AXES_INITIAL_STATE_FILENAME, AXES_OUTPUTS_FILENAME, AXES_INDEX_COLUMNS),
            ...
            if initial_state_filename == ELEMENTS_INITIAL_STATE_FILENAME:
                ...
                new_initial_state = elements_previous_outputs[~elements_previous_outputs.is_over.isna()]  # TODO : verifier ce que 軋 donne avec des None
            else:
                ...
                for t_senescwheat in range(t_caribu, t_caribu + SENESCWHEAT_TIMESTEP, SENESCWHEAT_TIMESTEP):
                    ...
                    for t_farquharwheat in range(t_senescwheat, t_senescwheat + SENESCWHEAT_TIMESTEP, FARQUHARWHEAT_TIMESTEP):
                        ...
                        for t_elongwheat in range(t_farquharwheat, t_farquharwheat + FARQUHARWHEAT_TIMESTEP, ELONGWHEAT_TIMESTEP):
                            ...
                            elongwheat_facade_.run(Tair, Tsoil, option_static=option_static, optimal_growth_option=True, measured_leaf_length=True) # zhao: use measured leaf length
    ...
    if generate_graphs:
        ...
        ax.set_xlabel('t')
        ax.set_ylabel(u'Photosynthetic efficiency (ｵmol C/ｵmol PARa)')
        ...
        ax.set_xlabel(u'Cumulative absorbed PAR (ｵmol.s$^{-1}$)')
        ax.set_ylabel(u'Cumulative photosynthesis (ｵmol C.s$^{-1}$)')
        ...
        C_usages['C_budget'] = (C_usages.Respi_roots + C_usages.Respi_shoot + C_usages.exudation + C_usages.Structure_roots + C_usages.Structure_shoot + C_usages.NS_phloem + C_usages.NS_other) / \
                               (C_usages.C_produced+1e-5)  # added 1e-5 to avoid division 0 error.

        #  Graph
        fig, ax = plt.subplots()
        ax.plot(C_usages.t, C_usages.Structure_shoot / (C_usages.C_produced+1e-5) * 100,
                label=u'Structural mass - Shoot', color='g')
        ax.plot(C_usages.t, C_usages.Structure_roots / (C_usages.C_produced+1e-5) * 100,
                label=u'Structural mass - Roots', color='r')
        ax.plot(C_usages.t, (C_usages.NS_phloem + C_usages.NS_other) / (C_usages.C_produced+1e-5) * 100, label=u'Non-structural C', color='darkorange')
        ax.plot(C_usages.t, (C_usages.Respi_roots + C_usages.Respi_shoot) / (C_usages.C_produced+1e-5) * 100, label=u'C loss by respiration', color='b')
        ax.plot(C_usages.t, C_usages.exudation / (C_usages.C_produced+1e-5) * 100, label=u'C loss by exudation', color='c')
...
if __name__ == '__main__':
    main(simulation_length=2000, forced_start_time=0,
         run_simu=True, run_postprocessing=True, generate_graphs=True, run_from_outputs=False, option_static=False,
         show_3Dplant=False, heterogeneous_canopy=False, METEO_FILENAME="meteo_kgit20240123-0422.csv")
