...
def main(stop_time, run_simu=True, make_graphs=False, farquhar_update_paramters={}, cn_update_parameters={}):
    if run_simu:
        ...
        senescwheat_facade_ = senescwheat_facade.SenescWheatFacade(g,
                                                                   ...
                                                                   shared_elements_inputs_outputs_df, option_static=False)
        # growthwheat
        ...
        update_parameters_farquharwheat.update(farquhar_update_paramters)
        ...
        from farquharwheat import parameters as far_par
        print('farquharwheat updated parameters:', far_par.__dict__['GAMMA25'], far_par.__dict__['PARAM_N']['S_surfacic_nitrogen']['TPU25'])
        ...
        update_cnwheat_parameters.update(cn_update_parameters)                          
        ...
        # print('k_starch in cnwheat:', cnwheat_facade_.population.plants[0].axes[0].grains.PARAMETERS.K_STARCH)
        ...
        try:
                    ...
                    for t_growthwheat in range(t_senescwheat, t_senescwheat + senescwheat_ts, growthwheat_ts):
                        ...
                        for t_farquharwheat in range(t_growthwheat, t_growthwheat + growthwheat_ts, farquharwheat_ts):
                            ...
                            for t_cnwheat in range(t_farquharwheat, t_farquharwheat + farquharwheat_ts, cnwheat_ts):
                                Tair, Tsoil = meteo.loc[t_cnwheat, ['air_temperature', 'air_temperature']]  # (zhao) acquire the temperature data in a finer granule for cnwheat
                                # run CNWheat
            ...
            print('Simulation run in ', str(datetime.timedelta(seconds=execution_time)))
    ...
    grain_C_mass =  ((all_organs_inputs_outputs[all_organs_inputs_outputs['organ']=='grains'].iloc[-1]['structure']+all_organs_inputs_outputs[all_organs_inputs_outputs['organ']=='grains'].iloc[-1]['starch'])* 1E-6 * cnwheat_model.EcophysiologicalConstants.C_MOLAR_MASS) / cnwheat_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
    
    grain_N_mass = (all_organs_inputs_outputs[all_organs_inputs_outputs['organ']=='grains'].iloc[-1]['proteins'] * 1E-6 * cnwheat_model.EcophysiologicalConstants.N_MOLAR_MASS) / cnwheat_model.Grains.AMINO_ACIDS_MOLAR_MASS_N_RATIO
    
    return [grain_C_mass+grain_N_mass]
    #print('final check: {}'.format(grain_C_mass+grain_N_mass))
