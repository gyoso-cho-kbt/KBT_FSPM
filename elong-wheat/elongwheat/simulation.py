...
#### zhao: add 'current_nb_leaves' to represent the current leaf number instead of using 'nb_leaves'
AXIS_INPUTS = ['SAM_temperature', 'delta_teq', 'teq_since_primordium', 'status', 'nb_leaves', 'GA', 'SAM_height', 'cohort', 'sum_TT', 'current_nb_leaves']
...
class Simulation(object):
    ...
    def run(self, Tair, Tsoil, optimal_growth_option, measured_leaf_length=False):
        """
        Run the simulation.
        ...
        try:
            for axis_id, axis_inputs in sorted(all_axes_inputs.items()):
                ...
                current_nb_leaves = curr_axis_outputs['current_nb_leaves']

                # height of the SAM
                self.logger.info('main stem all_sheath_internode_lengths:{}'.format(all_sheath_internode_lengths[axis_id]))
                ############## zhao: alter the 'nb_leaves' to 'current_nb_leaves' #######################
                # below_internode_lengths = all_sheath_internode_lengths[axis_id][nb_leaves]['cumulated_internode']
                below_internode_lengths = all_sheath_internode_lengths[axis_id][current_nb_leaves]['cumulated_internode']
                ##########################################################################################
                ...
                # zhao: alter the 'nb_leaves' to 'current_nb_leaves'
                init_leaf, curr_axis_outputs['current_nb_leaves'], curr_axis_outputs['status'], curr_axis_outputs['teq_since_primordium'] = model.calculate_SAM_primodia(axis_inputs['status'],curr_axis_outputs['teq_since_primordium'],curr_axis_outputs['delta_teq'], current_nb_leaves,curr_axis_outputs['cohort'])
                ...
                for i in range(0, init_leaf):
                    ...
                    hiddenzone_id = axis_id + tuple([1+i+curr_axis_outputs['current_nb_leaves'] - init_leaf])
                    #######################################################################################################
                    ################################## zhao: skip the intialization process if the hiddenzone_id already exists. Because the initialized hiddenzone may cause 'leaf_Lmax_em' None value error.####
                    if hiddenzone_id in all_hiddenzone_inputs.keys():
                        continue
                    ################################################################################################################
                ...
                #: Main Stem
                    ...
                    if curr_hiddenzone_outputs['leaf_is_growing']:
                        ...
                        if not prev_leaf_emerged:  #: Before the emergence of the previous leaf. Exponential-like elongation.
                            ...
                            ######################### zhao: add validity check ##########################################
                            if not hiddenzone_inputs['leaf_Lmax_em'] or not hiddenzone_inputs['leaf_L']:
                                print('Leaf_Lmax_em or Leaf_L for hiddenzone id {} has not yet been initialized: leaf_Lmax_em {}, leaf_L {}'.format(hiddenzone_id, hiddenzone_inputs['leaf_Lmax_em'], hiddenzone_inputs['leaf_L']))
                                continue
                            #############################################################################################
                            ...
                            leaf_L = hiddenzone_inputs['leaf_L'] + delta_leaf_L
                            # print("[elongwheat/simulation] hiddenzone_id: {}, leaf_L/delta_leaf_L: {}/{}".format(hiddenzone_id, leaf_L, delta_leaf_L))
                            ...
                            if not curr_hiddenzone_outputs['leaf_is_emerged']:
                                ...
                                if curr_hiddenzone_outputs['leaf_is_emerged']:  # Initialise lamina outputs
                                    ...
                                    if next_hiddenzone_id in all_hiddenzone_inputs:
                                        ...
                                        ################ zhao: add functioning using measured leaf values ####################################
                                        if not measured_leaf_length:
                                            next_hiddenzone_outputs['leaf_Lmax'] = model.calculate_leaf_Lmax(next_hiddenzone_inputs['leaf_L'])  #: Final leaf length
                                        else:
                                            print('[smulation] hiddenzone id: {}, hiddezon_input leaf_Lmax: {}, metamer10 leaf_Lmax: {}'.format(next_hiddenzone_id, next_hiddenzone_inputs['leaf_Lmax'], all_hiddenzone_inputs[(1, 'MS', 10)]['leaf_Lmax']))
                                            #########zhao: use inference value if input value is not avaliable####################
                                            if next_hiddenzone_inputs['leaf_Lmax']:
                                                next_hiddenzone_outputs['leaf_Lmax'] = next_hiddenzone_inputs['leaf_Lmax']
                                            else:
                                                next_hiddenzone_outputs['leaf_Lmax'] = model.calculate_leaf_Lmax(next_hiddenzone_inputs['leaf_L'])
                                            ######################################################################################
                    ...
                    if curr_hiddenzone_outputs['internode_is_growing']:
                        ...
                        if not prev_leaf_ligulated:
                            delta_internode_L = model.calculate_delta_internode_L_preL(phytomer_id, curr_hiddenzone_outputs['sucrose'], curr_hiddenzone_outputs['internode_L'],curr_hiddenzone_outputs['amino_acids'], curr_hiddenzone_outputs['mstruct'],curr_axis_outputs['delta_teq'], optimal_growth_option=True)
                            ...
                            if curr_axis_outputs['GA']:
                                ...
                                if not curr_hiddenzone_outputs['internode_is_visible']:
                                    ...
                                    if curr_hiddenzone_outputs['internode_is_visible']:  #: Initialise internode outputs
                                        ...
                                        self.outputs['elements'][visible_internode_id]['length'] = min(curr_hiddenzone_outputs['internode_Lmax'],model.calculate_emerged_internode_L(internode_L, internode_distance_to_emerge))
                                        self.outputs['elements'][hidden_internode_id]['length'] = internode_distance_to_emerge
                                    else:
                                        ############### zhao: key error fixing ##################
                                        if hidden_internode_id not in self.outputs['elements'].keys():
                                            new_internode = parameters.ElementInit().__dict__
                                            self.outputs['elements'][hidden_internode_id] = new_internode
                                        ###########################################################
                    ...
                    # zhao: send all the hiddenzone outputs
                    ...
                    if hiddenzone_inputs['internode_is_growing'] or curr_hiddenzone_outputs['leaf_is_growing'] or \
                        ...
                        self.outputs['hiddenzone'][hiddenzone_id] = curr_hiddenzone_outputs
                        self.logger.info('sent hiddenzone ids: {}'.format(self.outputs['hiddenzone'].keys()))
