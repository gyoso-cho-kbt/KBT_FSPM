...
class EcophysiologicalConstants:
    ...
    PROTEINS_MOLAR_MASS_N_RATIO = 0.151  #: Mean contribution of N in protein mass (Penning De Vries 1989) # zhao: the N mass in 1g proteins
    ...
    AMINO_ACIDS_MOLAR_MASS_C_RATIO = 0.38  #: (Penning De Vries 1989)  # zhao: the C mass in 1g AA
    PROTEINS_MOLAR_MASS_C_RATIO = 0.38  #: As for AA  # zhao: the C mass in 1g proteins
...
# def grain_filling_debugger(func):
    ...
    # def inner(ref, sucrose_phloem, mstruct_axis, T_effect_Vmax):
        # print('{}-{}: {}'.format(Grains.PARAMETERS.FILLING_INIT, Grains.PARAMETERS.FILLING_END, ref.age_from_flowering))
        # s_grain_starch = func(ref, sucrose_phloem, mstruct_axis, T_effect_Vmax)
...
class Grains(Organ):
    ...
    def calculate_RGR_Structure(sucrose_phloem, mstruct_axis, T_effect_growth):
        ...
        :rtype: float
        """
        return ((max(0., sucrose_phloem) / (mstruct_axis * Axis.PARAMETERS.ALPHA)) * Grains.PARAMETERS.VMAX_RGR) / ((max(0., sucrose_phloem) / (mstruct_axis * Axis.PARAMETERS.ALPHA)) +Grains.PARAMETERS.K_RGR) * T_effect_growth
    ...
    def calculate_S_proteins(S_grain_structure, S_grain_starch, amino_acids_phloem, sucrose_phloem, structural_dry_mass):
        ...
        :return: Synthesis of grain proteins (µmol` N) # zhao: the unit should be (µmol` N h-1)
        :rtype: float
        """
        if sucrose_phloem > 0:
...
class PhotosyntheticOrganElement(object):
    ...
    def calculate_triosesP_derivative(self, Photosynthesis, S_Sucrose, S_Starch, S_Amino_Acids):
        ...
        triosesP_consumption_AA = (S_Amino_Acids / EcophysiologicalConstants.AMINO_ACIDS_N_RATIO) * EcophysiologicalConstants.AMINO_ACIDS_C_RATIO  # the C consumption ratio for amino acids synthesis.
