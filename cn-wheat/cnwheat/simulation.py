...
class Simulation(object):
    ...
    ORGANS_STATE = ORGANS_STATE_PARAMETERS + MODEL_COMPARTMENTS_NAMES.get(model.Organ, [])  # ORGANS_STATE is shared by all the plant organs (e.g. roots, phloem, grains, etc.)
    ...
    def initialize(self, population, soils, Tair=12, Tsoil=12):
        ...
        # initial_conditions_mapping maps the (model_object, compartment) to its corresponding id, 
        # which is set in the _init_initial_conditions() function
        self.initial_conditions_mapping.clear()  

        # create new population and soils
        self.population.plants.extend(population.plants) # use the input population parameter.
        ...
        def _init_initial_conditions(model_object, index): # fetch the class through model_object, and returns the compartments_names in the fetched class.
            ...
            compartments_names = Simulation.MODEL_COMPARTMENTS_NAMES[class_]  # get the corresponding compartments for a class.
            ...
            for compartment_name in compartments_names:
                if hasattr(model_object, compartment_name):
                    self.initial_conditions_mapping[model_object][compartment_name] = index  # the 'index' is a record for the self.initial_conditions index. 'model_object' here is an instance of the class. 
        ...
        for soil in soils.values():  # note the soils referred here is the variable in the 'cnwheat_facade'. The 'soils' variable is a dictionary of soil entries whose key is the axis id.
    ...
    def run(self, show_progressbar=False):
        ...
        try:
            ...
            self._update_initial_conditions() # the real-time values are set here.
            ...
            # Important Note: the parameter 'y' here is not the paramter 'y0' passed in the 'solve_ivp' function, 
            # but a variable continuously undergoing evaluation according to the returned derivatives. 
            # 'y0' only gives the initial values of 'y', and has no further effect on 'y'.
            # Therefore, the 'y' can be used to track the evaluated status variables on the fly (i.e. fine granular tracking).
            sol = solve_ivp(fun=self._calculate_all_derivatives, t_span=self.time_grid, y0=self.initial_conditions, method='BDF', t_eval=np.array([self.time_step]), dense_output=False)
    ...
    def _update_initial_conditions(self):
        ...
        for model_object, compartments in self.initial_conditions_mapping.items():
            for compartment_name, compartment_index in compartments.items():
                # note that model_object here is an INSTANCE of a class rather than the class itself, thus
                # the actual value of 'compartment_name' is fetched from the instnace 'model_object' which is already initialized.
                # specifically, the 'getattr' function is able to access the member variable by variable name string instead of the dot operator.
    ...
    def _calculate_all_derivatives(self, t, y):
        ...
        if self.interpolate_forcings:
            ...
            self.population.calculate_aggregated_variables() # 'calculate_aggregated_variables' is implemented in every scale of the system, which mainly used for Nitrate caculation.
        ...
        # TODO: TEMP !!!!
        soil_contributors = []
        soil = self.soils[(1, 'MS')]  # zhao: the current implementation can only handle one culm.
        ...
        for plant in self.population.plants:
            ...
            for axis in plant.axes:
                ...
                # Phloem
                phloem_contributors = []  # a list of the physiology instances related to the phloem dynamic.
                        ...
                        if organ is not None:
                            for element in (organ.exposed_element, organ.enclosed_element):
                                if element is not None and element.green_area > 0:
                                    ...
                                    axis.Total_Transpiration += (element.Transpiration * element.nb_replications)                 # the calculated transpiration and total_transpiration are used for substrate distribution.
                ...
                if axis.grains is not None:
                    ...
                    axis.grains.structural_dry_mass = axis.grains.calculate_structural_dry_mass(axis.grains.structure) # axis.grains.structure is the Grain structural C mass (umol), axis.grains.structural_dry_mass is the estimated grain mass (g), which is equivalent to the mstruct in other organs.
